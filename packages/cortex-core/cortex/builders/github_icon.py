...
def build(config: Config, output: str | Path) -> Path:
    ...
    svg = (
        ...
        f'      <stop offset="0%"   stop-color="{primary}" stop-opacity="0.85"/>\n'
        f'      <stop offset="55%"  stop-color="{primary}" stop-opacity="0.30"/>\n'
        ...
        '    <animate attributeName="r"       values="32;44;32"     dur="2.6s" repeatCount="indefinite"/>\n'
        '    <animate attributeName="opacity" values="0.55;1;0.55" dur="2.6s" repeatCount="indefinite"/>\n'
        "  </circle>\n"
        f'  <circle cx="48" cy="48" r="28" fill="url(#ghDisc)" stroke="{primary}" stroke-width="1.5" stroke-opacity="0.7"/>\n'
